...
export const onNewMessage = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (afterMessages.length > beforeMessages.length) {
      ...
      const otherMembers = members.filter((uid: string) => uid !== sender);
 ...
 */
async function sendChatRoomCreatedNotification(fromUid: string, toUid: string) {
  try {
    // Get user profiles
 ...
 */
async function sendMessageNotification(recipientUid: string, senderUid: string, messageContent: string) {
  try {
    // Get sender profile
