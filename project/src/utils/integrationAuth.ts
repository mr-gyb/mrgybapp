export const getAuthUrl = (provider: string): string => {
  ...
  // Validate Google Client ID before using it
  const googleClientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
  if (!googleClientId || googleClientId === 'your_google_client_id_here' || googleClientId === 'placeholder') {
    console.error('⚠️ VITE_GOOGLE_CLIENT_ID is not configured. Please set it in your .env file.');
    throw new Error('Google OAuth client ID is not configured. Please check your environment variables.');
  }
  
  const authUrls: Record<string, string> = {
    'salesforce': `https://login.salesforce.com/services/oauth2/authorize?response_type=code&client_id=${import.meta.env.VITE_SALESFORCE_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=api`,
    'google': `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${googleClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=profile%20email`,
    ...
    'youtube': `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${import.meta.env.VITE_YOUTUBE_CLIENT_ID || googleClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=https://www.googleapis.com/auth/youtube.readonly+https://www.googleapis.com/auth/yt-analytics.readonly`,
